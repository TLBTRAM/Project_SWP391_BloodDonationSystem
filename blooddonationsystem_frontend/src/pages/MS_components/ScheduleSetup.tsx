...
const ScheduleSetup: React.FC = () => {
  ...
  const [registeredSchedules, setRegisteredSchedules] = useState<{accountScheduleId: number, scheduleDate: string}[]>([]);
  ...
  useEffect(() => {
    axios.get(`${baseURL}/api/schedules/open`, {
  ...
  const fetchRegistered = () => {
    axios.get(`${baseURL}/api/schedules/medicalstaff-registered`, {
      headers: { Authorization: `Bearer ${token}` }
    })
      .then(res => {
        setRegisteredSchedules(res.data.map((s: any) => ({ accountScheduleId: s.accountScheduleId, scheduleDate: s.scheduleDate })));
      })
  ...
  const handleSave = async () => {
    ...
    const schedule = openSchedules.find(s => s.scheduleDate === formattedDate);
    if (!schedule) {
    ...
    try {
      ...
      setToastMsg("Đăng ký thất bại: " + (error.response?.data || error.message));
    }
  };
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      await axios.delete(`${baseURL}/api/schedules/medicalstaff-registered/${deleteId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchRegistered();
      setDeleteId(null);
  ...
  return (
    <div className="schedule-setup-wrapper">
        ...
        {registeredSchedules.length === 0 ? (
          ...
          <div className="registered-schedule-list">
            {registeredSchedules
              ...
              .map((item) => (
                <div className="registered-schedule-item" key={item.accountScheduleId}>
                  <span>{format(new Date(item.scheduleDate), "dd/MM/yyyy")}</span>
                  <button className="delete-btn" onClick={() => handleDelete(item.accountScheduleId)}>
                    Xóa
                  </button>
                </div>
            ))}
      ...
      {deleteId && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>Bạn có chắc chắn muốn <span style={{color:'#FF204E', fontWeight:'bold'}}>xóa</span> lịch này không?</p>
            <div className="modal-actions">
              <button onClick={confirmDelete} className="modal-confirm">Xóa</button>
              <button onClick={() => setDeleteId(null)} className="modal-cancel">Hủy</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Toast thông báo */}
      {toastMsg && (
        <div className="admin-toast-success">{toastMsg}</div>
      )}
    </div>
  );
};
